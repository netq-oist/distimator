...
epspArr = np.zeros((np.shape(qVecArr)[0],3))

for k in np.arange(np.shape(qVecArr)[0]):
    epsArrTemp = np.zeros(3)
    
    for i in np.arange(3):
        xVecTempL     = xVecArrEst[k,:] 
        xVecTempL[i] -= epsxArr[i]
        
        xVecTempR     = xVecArrEst[k,:] 
        ...
        epsArrTemp[i] = max(rateEpsL, rateEpsR)    
        
    epspArr[k,:] = epsArrTemp

#%% Calculate empirical Hoeffding bounds

delta = np.longdouble(2.*np.exp(-2.*np.array([totSamples1,totSamples2,totSamples3])*epspArr**2.))
...
norm = pltco.TwoSlopeNorm(vmin=-17, vcenter=-2, vmax=0.)
cmap = pltco.LinearSegmentedColormap.from_list("", ["blue","yellow","red"])
...
np.savetxt('belldiagonal_N1e5_Allepsx1e-2.txt', result, delimiter='\t', newline='\n',\
