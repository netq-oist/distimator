...
wArray = np.linspace(0.0,0.66,20,False)[1:]
...
wInvLeft5  = np.copy(wInvArr5) - epsw
wInvRight5 = np.copy(wInvArr5) + epsw

rateErr5 = np.zeros((len(wArray),2))

wInvLeft6  = np.copy(wInvArr6) - epsw
wInvRight6 = np.copy(wInvArr6) + epsw

rateErr6 = np.zeros((len(wArray),2))
...
for k in np.arange(len(wArray)):
  ...
  rateErr5[k,:] = np.array([abs(rateL - rateExpArr5[k]), abs(rateR - rateExpArr5[k])])
...
for k in np.arange(len(wArray)):
  ...
  rateErr6[k,:] = np.array([abs(rateL - rateExpArr6[k]), abs(rateR - rateExpArr6[k])])
...
fig, ax = plt.subplots()
ax.errorbar(wArray, rateExpArr5, yerr=[rateErr5[:,0],rateErr5[:,1]], fmt='o', ls='none',\
...
fig, ax = plt.subplots()
ax.errorbar(wArray, rateExpArr6, yerr=[rateErr6[:,0],rateErr6[:,1]], fmt='o', ls='none',\
...
epsRateExpected5 = np.zeros((len(wArrayFit),2))
epsRateExpected6 = np.zeros((len(wArrayFit),2))
...
for k in np.arange(len(wArrayFit)):
    ...
    epsRateExpected5[k] = np.array([abs(rateL - rate), abs(rateR - rate)])
...
for k in np.arange(len(wArrayFit)):
    ...
    epsRateExpected6[k] = np.array([abs(rateL - rate), abs(rateR - rate)])

deltaRateExpected105 = np.exp(-2*(10**5)*epsRateExpected5[:,0]**2) \
                        + np.exp(-2*(10**5)*epsRateExpected5[:,1]**2)
deltaRateExpected106 = np.exp(-2*(10**6)*epsRateExpected6[:,0]**2) \
                        + np.exp(-2*(10**6)*epsRateExpected6[:,1]**2)

#%% simulated failure probability

deltaRate5 = np.exp(-2*len(tArray5)*rateErr5[:,0]**2) \
            + np.exp(-2*len(tArray5)*rateErr5[:,1]**2)
            
deltaRate6 = np.exp(-2*len(tArray6)*rateErr6[:,0]**2) \
            + np.exp(-2*len(tArray6)*rateErr6[:,1]**2)
...
result5 = np.hstack((wArray[...,None],rateExpArr5[...,None],\
                    wInvArr5[...,None],rateErr5,deltaRate5[...,None]))
result6 = np.hstack((wArray[...,None],rateExpArr6[...,None],\
                    wInvArr6[...,None],rateErr6,deltaRate6[...,None]))
